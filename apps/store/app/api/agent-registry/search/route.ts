...
import { logServerError } from "@cotana/config/runtime";
...
export async function GET(request: NextRequest) {
  try {
    ...
    void recordAgentRegistryEvaluationLog(evaluation).catch((error) =>
      logServerError("Failed to persist agent registry evaluation log.", error, {
        route: "/api/agent-registry/search",
        query
      }),
    );
    ...
    logServerError("Agent registry search failed.", error, {
      route: "/api/agent-registry/search",
      query: request.nextUrl.searchParams.get("q") ?? ""
    });
    return NextResponse.json({ error: "Agent registry search failed." }, { status: 500 });
  }
}
